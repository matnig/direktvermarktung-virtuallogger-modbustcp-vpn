...
- **Home Assistant MQTT Auto-Discovery** — virtual variables with `haEnabled` are automatically published as HA entities via the [MQTT Discovery protocol](https://www.home-assistant.io/integrations/mqtt/#mqtt-discovery). Creating, updating, or deleting a variable in the web UI immediately syncs the entity in Home Assistant — no manual YAML configuration required. Writable variables expose a command topic so they can be controlled from the HA dashboard. Entity types (`sensor`, `number`, `binary_sensor`) and device classes (power, energy, temperature, …) are inferred automatically from the variable's data type and unit. State values are published only on change for efficiency.
...
- **HA Discovery — entity types**: Only virtual variables are exposed as HA entities. Internal registers, external registers, and watchdogs are not directly discoverable (use publish rules or virtual variables as intermediaries).
- **HA Discovery — device class override**: The device class is inferred from the variable's unit string (e.g. `W` → `power`, `°C` → `temperature`, `%` → `humidity`). There is no manual override in the UI; assign a different unit if the inferred class is incorrect.
- **HA Discovery — state persistence**: Discovery config messages are published with `retain: true` so HA picks them up after restart. State values are also retained. However, variable runtime values are in-memory and reset on server restart; HA will show the last retained value until a new poll cycle updates it.
